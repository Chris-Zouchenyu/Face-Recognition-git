...
class SiameseDataset(Dataset):
    def __init__(self, input_shape, lines, labels, random, autoaugment_flag=False):
        ...
        if self.autoaugment_flag:# Autoaugument: 自动增强 
            self.resize_crop = RandomResizedCrop(input_shape)# 随机裁剪图像的一部分，并将这部分缩放到给定的 input_shape 大小
            self.policy      = ImageNetPolicy()
            
            self.resize      = Resize(input_shape[0] if input_shape[0] == input_shape[1] else input_shape)# 将图像缩放到指定的input_shape
            self.center_crop = CenterCrop(input_shape)# 中心裁剪，裁剪到input_shape的大小
    ...
    def __getitem__(self, index):
        ...
        #   首先选取三张类别相同的图片
        #------------------------------------------#
        c               = random.randint(0, self.types - 1)# 从所有标签中随机选一个标签 max(labels)
        selected_path   = self.train_lines[self.train_labels[:] == c]# 通过标签确定路径（包含很多图片）
        while len(selected_path)<3:# 如果找到的图像数量少于3张，则重新选择标签并筛选，直到找到至少3张图像
            c               = random.randint(0, self.types - 1)
            selected_path   = self.train_lines[self.train_labels[:] == c]

        image_indexes = random.sample(range(0, len(selected_path)), 3)# 从筛选出的图像路径中随机选择3张图像的索引
        #------------------------------------------#
        #   取出两张类似的图片
        #   对于这两张图片，网络应当输出1
        #------------------------------------------#
